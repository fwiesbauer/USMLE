...
28. **Direct storage upload** — Moved PDF upload from the Vercel serverless function to direct browser-to-Supabase-Storage upload, bypassing Vercel's 4.5 MB body size limit (Hobby plan). The API route now receives only the storage path and downloads the PDF from storage for processing. Added storage RLS policies for authenticated users.
